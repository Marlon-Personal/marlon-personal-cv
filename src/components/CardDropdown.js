...
const CardDropdown = ({ mainImage, data, sectionTitle }) => {
    ...
    return (
        <>
            <div id={`drodpdown-card-${sectionTitle.replace(/(?:^\w|[A-Z]|\b\w|\s+)/g, function (match, index) {
            ...
            })}`} className="px-8 pt-8 mt-16 md:mt-24 h-screen relative">
                <h2 className="text-4xl text-gray-800 text-center">
                    ...
                    <div className="h-1/3 md:h-2/3 flex items-center justify-center md:mt-0 absolute bottom-0 z-0 md:left-1/2 md:bottom-6 md:right-6 left-0 right-0">
